...
const httpFinancialReport = axios.create({
    baseURL: `${getApiOrigin()}/api/admin/financial-report`,
    headers: { "Content-Type": "application/json" },
});

// Request interceptor - add token
httpFinancialReport.interceptors.request.use(
    createRequestInterceptor(getAuthToken),
    (error) => Promise.reject(error)
);

// Response interceptor
httpFinancialReport.interceptors.response.use(
    (res) => res,
    createErrorInterceptor(true) // true = isAdmin
);

export default httpFinancialReport;
