...
const httpVoucher = axios.create({
    baseURL: `${getApiOrigin()}/api/admin/voucher`,
    headers: { "Content-Type": "application/json" },
});

httpVoucher.interceptors.request.use(
    createRequestInterceptor(getAuthToken),
    (error) => Promise.reject(error)
);

httpVoucher.interceptors.response.use(
    (res) => res,
    createErrorInterceptor(true)
);

export default httpVoucher;
